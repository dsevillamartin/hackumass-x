...
const process = async (storage, documents, userId) => {
  ...
  const fileName = `user-${userId}.ics`;

  fs.writeFileSync(`${__dirname}/${fileName}`, calendar.toString());

  const fileInput = sdk.InputFile.fromPath(
    `${__dirname}/${fileName}`,
    fileName
  );

  try {
    await storage.deleteFile('6370291502dec4fa9083', fileName);
    console.log(`* ${userId} > old deleted`);
  } catch (e) {}

  await storage.createFile('6370291502dec4fa9083', userId, fileInput);

  console.log(`* ${userId} > uploaded (${events.length})`);
};
...
module.exports = async function (req, res) {
  ...
  const query = userId
    ? [sdk.Query.equal('user_id', userId), sdk.Query.limit(100)]
    : [sdk.Query.limit(100)];
  const allUsers = userId ? null : await users.list();
  const documents = await database.listDocuments(
    '636f5b6eafb4f87bc2a5',
    '636f5ba37013c90ca3fb',
    query
  );
